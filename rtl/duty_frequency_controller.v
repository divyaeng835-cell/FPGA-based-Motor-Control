...
module duty_frequency_controller #(
    ...
    // =========================================================================
    // Sine ROM: 256 x 8-bit unsigned  (sin*127 + 128)
    // FIX: hardcoded literal values instead of $rtoi($sin(...)) in an initial
    // block. The old form used real-number math functions to build the ROM,
    // which Vivado tolerates at elaboration time but Cadence Genus rejects
    // during ASIC synthesis (no real/floating-point math in synthesizable
    // RTL). These are the same sin(2*pi*i/256)*127+128 values, precomputed
    // and individually verified against the mathematical reference.
    ...
    // 120 deg and 240 deg offsets in 256-step circle
